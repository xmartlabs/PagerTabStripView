...
extension PagerStyle where Self == BarStyle {
    public static func bar(
        ...
        pagerAnimation: Animation = .default,
        indicatorViewHeight: CGFloat = 10,
        indicatorView: @escaping () -> some View = { Rectangle() }
    ) -> BarStyle {
        return BarStyle(
...
extension PagerStyle where Self == BarButtonStyle {
    public static func scrollableBarButton(
        placedInToolbar: Bool = false,
        pagerAnimation: Animation = .default,
        ...
        @ViewBuilder indicatorView: @escaping () -> some View = { Rectangle().fill(.blue) }
    ) -> BarButtonStyle {
